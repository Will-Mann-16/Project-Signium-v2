...
export function fetchStudentsMajor(){
  return dispatch => {
    ...
    axios.get(scriptsDirectory + "students/read", {params: { house: house }}).then((response) =>{
      dispatch({type: "FETCH_STUDENTS_MAJOR_FULFILLED", payload: response.data});
    }).catch((err) =>{
...
export function updateStudentLocation(ids, location){
  return dispatch => {
    ...
    axios.get(scriptsDirectory + "students/update-location",  {params: { ids: ids, location: location, house: house }}).then((response) =>{
      if(response.data.success){
        socket.emit("socket-client-server-redraw-minor");
